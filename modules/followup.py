...
def get_condominios_com_contagem(clientes_collection, forcar_atualizacao=False):
    ...
    if precisa_atualizar:
        try:
            pipeline = [
                {"$match": {"seguiu_ativacao": {"$ne": "Sim"}, "restritivo": {"$ne": "Sim"}, "status_followup": {"$ne": "removido"}, "condominio_nome": {"$ne": None, "$ne": " "}}},
                {"$group": {"_id": "$condominio_nome", "count": {"$sum": 1}}},
                {"$sort": {"_id": 1}}
            ]
    ...
    with st.expander(expander_title, expanded=False):
        ...
        with col1:
            ...
            if cliente.get("bloco") or cliente.get("apartamento"):
                ...
                unidade_texto = []
                if bloco: unidade_texto.append(f"Bloco {bloco}")
                if apto: unidade_texto.append(f"Apto {apto}")
        ...
        with col2:
            ...
            if st.button("✅ Registrar Touch", key=f"touch_{key_base}", type="secondary"):
                ...
                clientes_collection.update_one(
                    {"_id": cliente["_id"]},
                    {"$set": {"touch_count": novo_count}, "$push": {"touch_history": {"timestamp": timestamp, "by": nome_usuario, "notes": "Touch registrado via Follow-up"}}}
                )
            ...
            if st.session_state.get("editando_followup") == _id:
                with st.form(f"form_edit_{key_base}"):
                    ...
                    data_para_exibicao = " "
                    if data_banco_orig:
                        try: data_para_exibicao = datetime.strptime(data_banco_orig, "%Y-%m-%d").strftime("%d/%m/%Y")
                        except: data_para_exibicao = data_banco_orig

                    nova_data_exibicao = st.text_input("Nova data de follow-up (DD/MM/AAAA):", value=data_para_exibicao, key=f"input_{key_base}")
                        ...
                        if st.form_submit_button("💾 Salvar"):
                            ...
                            if nova_data_exibicao.strip():
                                if re.match(r'\d{2}/\d{2}/\d{4}', nova_data_exibicao):
                                    ...
                                    return
                            clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"retorno_agendado": nova_data_banco}})
            ...
            with col_obs:
                ...
                nova_observacao = st.text_area(" ", value=obs_atual, placeholder="Ex: Cliente quer mais 3 pontos na semana que vem.", key=f"obs_{key_base}")
            with col_btns:
                if st.button("💾 Salvar", key=f"salvar_obs_{key_base}"):
                    clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"observacoes_followup": nova_observacao}})
                    st.success("✅ Observações salvas!")
                    st.rerun()
                if st.button("🚫 Remover da Lista", key=f"remover_{key_base}", type="secondary"):
                    clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"status_followup": "removido"}})
...
def formatar_ultimo_touch(cliente):
    ...
    if not touch_history: return "🆕 Nunca contactado"
    try:
        ultimo_ts = max([t.get("timestamp", " ") for t in touch_history])
        if not ultimo_ts: return "🆕 Nunca contactado "
        data_ultimo = datetime.fromisoformat(ultimo_ts.replace("Z", "+00:00"))
        agora = datetime.now(timezone.utc)
        diff = agora - data_ultimo
        dias, horas, minutos = diff.days, diff.seconds // 3600, (diff.seconds % 3600) // 60
        
        if dias == 0:
            if horas == 0:
                tempo_str = "agora mesmo" if minutos == 0 else f"há {minutos} min"
            else: tempo_str = f"há {horas}h"
        elif dias == 1: tempo_str = "ontem"
        elif dias < 7: tempo_str = f"há {dias} dias"
        elif dias < 30: tempo_str = f"há {dias//7} semana{'s' if dias//7 > 1 else ''}"
        else: tempo_str = data_ultimo.strftime("%d/%m/%Y")
...
def formatar_data_cadastro(data_cad):
    ...
    if data_cad is None: return "N/A"
    if isinstance(data_cad, str): return data_cad[:10] if len(data_cad) > 10 else data_cad
    if isinstance(data_cad, datetime): return data_cad.strftime("%Y-%m-%d")
...
def render_painel_ligacoes(clientes_collection, is_admin, usuario_atual):
    ...
    if is_admin:
        ...
        todos_atendentes = clientes_collection.distinct("cadastrado_por", {"seguiu_ativacao": {"$ne": "Sim"}, "restritivo": {"$ne": "Sim"}, "status_followup": {"$ne": "removido"}})
        ...
        with st.expander("⚙️ Configurar Modo Delegação", expanded=not modo_delegacao_ativo):
            ...
            with col_del1:
                modo_delegacao = st.toggle("🔄 Ativar", value=modo_delegacao_ativo, help="Ative para permitir que um atendente específico (ou todos) vejam todos os clientes do follow-up.", key="toggle_delegacao")
                st.session_state.modo_delegacao_ativo_sessao = modo_delegacao
                modo_delegacao_ativo = modo_delegacao
            with col_del2:
                if modo_delegacao:
                    atendente_delegado = st.selectbox("👤 Quem recebe todos os clientes:", options=opcoes_delegacao, index=opcoes_delegacao.index(atendente_delegado) if atendente_delegado in opcoes_delegacao else 0, key="select_atendente_delegado")
                    st.session_state.atendente_delegado_sessao = atendente_delegado
            with col_del3:
                persistir = st.checkbox("💾 Salvar configuração (persistente)", value=st.session_state.get("persistir_delegacao", bool(config_banco)), help="Se marcado, a configuração permanece ativa mesmo após reiniciar o sistema.", key="check_persistir")
                st.session_state.persistir_delegacao = persistir
                if modo_delegacao:
                    if atendente_delegado == "Todos os atendentes": st.info("💡 Todos os atendentes logados terão acesso à carteira completa.")
                    else: st.info(f"💡 Apenas **{atendente_delegado}** terá acesso à carteira completa.")
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("💾 Aplicar Configuração", use_container_width=True, type="primary"):
                    if persistir:
                        clientes_collection.database["configuracoes"].update_one({"tipo": "modo_delegacao"}, {"$set": {"tipo": "modo_delegacao", "ativo": modo_delegacao, "atendente": atendente_delegado, "ativado_por": usuario_atual, "data_ativacao": datetime.now(timezone.utc).isoformat()}}, upsert=True)
    ...
    with col_f1:
        filtro_touch_tipo = st.selectbox("Quantidade de ligações:", options=["Todos", "Nunca ligado (0)", "1-3 ligações", "4-6 ligações", "7-10 ligações", "Mais de 10", "Personalizado"], index=0, key="filtro_touch_tipo")
    with col_f2:
    ...
    with col_f3:
        filtro_periodo = st.selectbox("Último contato:", options=["Qualquer período", "Hoje", "Ontem", "Últimos 3 dias", "Última semana", "Últimos 15 dias", "Último mês", "Mais de 1 mês", "Nunca contactado"], index=0, key="filtro_periodo")
    with col_f4:
        filtro_nao_perturbar = st.checkbox("☑️ Incluir 'Não Perturbar'", value=False, help="Se marcado, mostra também clientes em período de não perturbar", key="filtro_nao_perturbar")

    st.markdown("#### ⚡ Ações em Lote Selecionados")
    col_acoes1, col_acoes2, col_acoes3, col_acoes4 = st.columns([1, 1, 1, 2])
    with col_acoes1: st.caption("Após selecionar clientes:")
    with col_acoes2:
        if st.button("📅 Tentar em 3 dias", key="acao_3dias", use_container_width=True): st.session_state.acao_lote = "tentar_3_dias"
    with col_acoes3:
        if st.button("🚫 Não perturbar 6m", key="acao_6meses", use_container_width=True): st.session_state.acao_lote = "nao_perturbar_6m"
    with col_acoes4:
        if st.button("❌ Remover da lista", key="acao_remover", use_container_width=True, type="secondary"): st.session_state.acao_lote = "remover"
    st.divider()

    col1, col2, col3 = st.columns([2, 2, 2])
    usuario_pode_ver_todos = is_admin or (modo_delegacao_ativo and (atendente_delegado == "Todos os atendentes" or atendente_delegado == usuario_atual))
    with col1:
        if usuario_pode_ver_todos:
            atendentes = clientes_collection.distinct("cadastrado_por", {"seguiu_ativacao": {"$ne": "Sim"}, "restritivo": {"$ne": "Sim"}, "status_followup": {"$ne": "removido"}})
            atendentes = [a for a in atendentes if a]
            atendentes.insert(0, "Todos")
            filtro_atendente = st.selectbox("Filtrar por atendente:", options=atendentes, index=0, key="painel_atendente")
        else:
            filtro_atendente = usuario_atual
            st.write(f"**Atendente:** {usuario_atual}")
    with col2:
        ordenacao = st.selectbox("Ordenar por:", options=["Data de cadastro (mais recente)", "Data de cadastro (mais antiga)", "Nome (A-Z)", "Touch count (mais touches primeiro)", "Touch count (menos touches primeiro)", "Último touch (mais recente)", "Último touch (mais antigo)"], index=0, key="painel_ordenacao")
    with col3:
        limite = st.number_input("Quantidade por página:", min_value=10, max_value=200, value=50, step=10, key="painel_limite")
    ...
    with col_condominio[0]:
        if st.button("🔄 Atualizar Lista de Condomínios", key="btn_atualizar_cond_painel", help="Atualiza a lista de condomínios"):
            if "condominios_cache_followup" in st.session_state: del st.session_state["condominios_cache_followup"]
            if "condominios_cache_timestamp_followup" in st.session_state: del st.session_state["condominios_cache_timestamp_followup"]
            st.rerun()
        condominios_opcoes = get_condominios_com_contagem(clientes_collection)
        opcoes_display = list(condominios_opcoes.keys())
        filtro_condominio_painel = st.multiselect("Condomínio:", options=opcoes_display, default=[], key="painel_filtro_condominio")

    query = {"seguiu_ativacao": {"$ne": "Sim"}, "restritivo": {"$ne": "Sim"}, "status_followup": {"$ne": "removido"}}
    if not usuario_pode_ver_todos: query["cadastrado_por"] = usuario_atual
    elif filtro_atendente != "Todos": query["cadastrado_por"] = filtro_atendente
    ...
    if filtro_condominio_painel and "Todos" not in filtro_condominio_painel:
        ...
        for opcao in filtro_condominio_painel:
            ...
            if nome_real != "Todos": condominios_selecionados.append(nome_real)
        if condominios_selecionados: query["condominio_nome"] = {"$in": condominios_selecionados}

    if filtro_touch_tipo == "Nunca ligado (0)": query["touch_count"] = {"$eq": 0}
    elif filtro_touch_tipo == "1-3 ligações": query["touch_count"] = {"$gte": 1, "$lte": 3}
    elif filtro_touch_tipo == "4-6 ligações": query["touch_count"] = {"$gte": 4, "$lte": 6}
    elif filtro_touch_tipo == "7-10 ligações": query["touch_count"] = {"$gte": 7, "$lte": 10}
    elif filtro_touch_tipo == "Mais de 10": query["touch_count"] = {"$gt": 10}
    elif filtro_touch_tipo == "Personalizado": query["touch_count"] = {"$gte": touch_min, "$lte": touch_max}

    hoje = datetime.now(timezone.utc)
    hoje_str = hoje.strftime("%Y-%m-%d")
    if not filtro_nao_perturbar:
        query["$or"] = [{"retorno_agendado": {"$exists": False}}, {"retorno_agendado": " "}, {"retorno_agendado": {"$lte": hoje_str}}]

    if filtro_periodo != "Qualquer período" and filtro_periodo != "Nunca contactado":
        if filtro_periodo == "Hoje": data_limite = hoje.replace(hour=0, minute=0, second=0)
        elif filtro_periodo == "Ontem": data_limite = (hoje - timedelta(days=1)).replace(hour=0, minute=0, second=0)
        elif filtro_periodo == "Últimos 3 dias": data_limite = hoje - timedelta(days=3)
        elif filtro_periodo == "Última semana": data_limite = hoje - timedelta(days=7)
        elif filtro_periodo == "Últimos 15 dias": data_limite = hoje - timedelta(days=15)
        elif filtro_periodo == "Último mês": data_limite = hoje - timedelta(days=30)
        elif filtro_periodo == "Mais de 1 mês": data_limite = hoje - timedelta(days=30)
        if filtro_periodo != "Mais de 1 mês": query["touch_history.timestamp"] = {"$gte": data_limite.isoformat()}
    elif filtro_periodo == "Nunca contactado":
        query["$or"] = [{"touch_history": {"$exists": False}}, {"touch_history": {"$size": 0}}]

    sort_field = "data_cadastro"
    sort_direction = -1
    if ordenacao == "Data de cadastro (mais antiga)": sort_direction = 1
    elif ordenacao == "Nome (A-Z)": sort_field, sort_direction = "nome_completo", 1
    elif ordenacao == "Touch count (mais touches primeiro)": sort_field, sort_direction = "touch_count", -1
    elif ordenacao == "Touch count (menos touches primeiro)": sort_field, sort_direction = "touch_count", 1
    elif ordenacao == "Último touch (mais recente)": sort_field, sort_direction = "touch_history.timestamp", -1
    elif ordenacao == "Último touch (mais antigo)": sort_field, sort_direction = "touch_history.timestamp", 1
            ...
            if touch_history:
                ...
                if ultimo_touch:
                    try:
                        ...
                        if data_ultimo < data_limite: clientes_filtrados.append(c)
                    except: pass
            else: clientes_filtrados.append(c)
    ...
    if modo_delegacao_ativo and not is_admin:
        if atendente_delegado == usuario_atual: st.success(f"🔄 **Modo Delegação Ativo** - Você está vendo {len(clientes)} cliente(s) de TODOS os atendentes.")
        else: st.info(f"✅ {len(clientes)} cliente(s) encontrado(s) para ligação.")
    else: st.success(f"✅ {len(clientes)} cliente(s) encontrado(s) para ligação!")

    st.markdown("---")
    col_exp1, col_exp2 = st.columns([1, 3])
    with col_exp1: st.markdown("### 📥 Exportar para Impressão")
    with col_exp2:
        dados_export = []
        for c in clientes:
            touch_history = c.get("touch_history", [])
            ...
            if touch_history:
                ultimo_ts = max([t.get("timestamp", " ") for t in touch_history])
                try: ultimo_contato = datetime.fromisoformat(ultimo_ts.replace("Z", "+00:00")).strftime("%d/%m/%Y %H:%M")
                except: ultimo_contato = ultimo_ts
            condominio_info = " "
            if c.get("condominio_nome"): condominio_info = c.get("condominio_nome", " ")
            if c.get("bloco") or c.get("apartamento"):
                bloco, apto = c.get("bloco", " "), c.get("apartamento", " ")
                if bloco: condominio_info += f" - Bloco {bloco}" if condominio_info else f"Bloco {bloco}"
                if apto: condominio_info += f" - Apto {apto}"
            dados_export.append({"Data Cadastro": c.get("data_cadastro", "N/A"), "Cadastrado Por": c.get("cadastrado_por", "N/A"), "Nome Completo": c.get("nome_completo", "N/A"), "Telefone": c.get("celular", "N/A"), "Condomínio/Unidade": condominio_info if condominio_info else "N/A", "Observações": c.get("observacoes_followup", " ").replace("\n", " "), "Toques": c.get("touch_count", 0), "Último Contato": ultimo_contato, "Retorno Agendado": c.get("retorno_agendado", "Imediato"), "Origem": c.get("origem", "N/A"), "Plano": c.get("plano_escolhido", "N/A")})
        ...
        with col_csv:
            st.download_button(label="📊 Excel/CSV (.csv)", data=csv_data, file_name=f"ligacoes_followup_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M')}.csv", mime="text/csv", use_container_width=True, help="Abre diretamente no Excel.")
        with col_txt:
            texto_impressao = "📞 LISTA DE LIGAÇÕES - FOLLOW UP\nGerado em: {}\nFiltros: {} | {}\nAtendente: {}\n{}\n\n".format(datetime.now(timezone.utc).strftime('%d/%m/%Y %H:%M'), filtro_touch_tipo, filtro_periodo, filtro_atendente if usuario_pode_ver_todos else usuario_atual, "=" * 80)
            if modo_delegacao_ativo: texto_impressao += f"⚠️ MODO DELEGAÇÃO: {atendente_delegado}\n"
            for i, c in enumerate(dados_export, 1):
                texto_impressao += f"{i}. {c['Nome Completo']} (Toques: {c['Toques']})\n📱 {c['Telefone']}\n"
                if c['Condomínio/Unidade'] != "N/A": texto_impressao += f"🏢 {c['Condomínio/Unidade']}\n"
                texto_impressao += f"📅 Cadastro: {c['Data Cadastro']} | Último contato: {c['Último Contato']}\n🔄 Retorno agendado: {c['Retorno Agendado']}\n📝 Obs: {c['Observações'][:80]}{'...' if len(c['Observações']) > 80 else ''}\n{'-' * 80}\n"
            st.download_button(label="📝 Texto (.txt)", data=texto_impressao, file_name=f"ligacoes_followup_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M')}.txt", mime="text/plain", use_container_width=True, help="Formato texto para impressão rápida.")
    st.markdown("---")
    st.markdown("### 🎯 Painel de Ligações")
    st.caption("Use os botões para registrar touch, adicionar observações ou agendar retorno.")
    st.markdown("""
     <style>
    .painel-card { background-color: #f8f9fa; border-radius: 10px; padding: 15px; margin-bottom: 10px; border-left: 5px solid #007bff; }
    .painel-card:hover { background-color: #e9ecef; }
    .telefone-destaque { font-size: 1.3em; font-weight: bold; color: #28a745; }
    .nao-perturbar { border-left: 5px solid #dc3545 !important; background-color: #f8d7da !important; }
    .data-cadastro { font-size: 0.85em; color: #6c757d; }
    .ultimo-touch { font-size: 0.8em; font-weight: 500; margin-top: 2px; }
    .condominio-info { font-size: 0.85em; color: #17a2b8; font-weight: 500; }
     </style>
    ...
    for idx, cliente in enumerate(clientes, 1):
        ...
        condominio_nome, bloco, apartamento = cliente.get("condominio_nome", " "), cliente.get("bloco", " "), cliente.get("apartamento", " ")
        condominio_display = " "
        if condominio_nome:
            condominio_display = f"🏢 {condominio_nome}"
            if bloco or apartamento:
                unidade_parts = []
                if bloco: unidade_parts.append(f"Bloco {bloco}")
                if apartamento: unidade_parts.append(f"Apto {apartamento}")
                condominio_display += f" - {' / '.join(unidade_parts)}"
        em_nao_perturbar = retorno_agendado and retorno_agendado > hoje_str
        
        if touch_count == 0: badge_touch = "🆕"
        elif touch_count <= 3: badge_touch = f"🟢 {touch_count}"
        elif touch_count <= 6: badge_touch = f"🟡 {touch_count}"
        elif touch_count <= 10: badge_touch = f"🟠 {touch_count}"
        else: badge_touch = f"🔴 {touch_count}"
        ...
        with st.container():
            ...
            with col_check:
                ...
                if selecionado: selecionados.append(_id)
            with cols_dados:
                ...
                with cols[1]:
                    ...
                    if condominio_display: st.caption(f"<span class='condominio-info'>{condominio_display}</span>", unsafe_allow_html=True)
                    if em_nao_perturbar: st.caption(f"🔕 Retorno: {datetime.strptime(retorno_agendado, '%Y-%m-%d').strftime('%d/%m/%Y')}")
                with cols[2]:
                ...
                with cols[4]:
                    if obs: st.caption(f"📝 {obs[:40]}{'...' if len(obs) > 40 else ''}")
                    else: st.caption("_Sem observações_")
                with cols[5]:
                    if st.button("✋ Touch", key=f"painel_touch_{_id}", use_container_width=True, type="primary"):
                        novo_count = touch_count + 1
                        nome_usuario = st.session_state.get("nome_usuario", "Anônimo")
                        timestamp = datetime.now(timezone.utc).isoformat()
                        clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"touch_count": novo_count}, "$push": {"touch_history": {"timestamp": timestamp, "by": nome_usuario, "notes": "Touch registrado via Painel de Ligações"}}})
    ...
    if st.session_state.get(f"mostrar_acoes_{_id}", False):
        with st.form(key=f"form_acoes_painel_{_id}"):
            ...
            with col_ac1:
                acao_obs = st.text_area("Observação:", value=obs, height=80, key=f"obs_acao_{_id}")
            with col_ac2:
                st.markdown(" &nbsp; ")
                if st.form_submit_button("💾 Salvar Obs", use_container_width=True):
                    clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"observacoes_followup": acao_obs}})
                    st.success("✅ Observação salva!")
                    st.session_state[f"mostrar_acoes_{_id}"] = False
                    st.rerun()
            with col_ac3:
                st.markdown("**Agendar Retorno:**")
                dias_retorno = st.selectbox("Daqui a:", options=[3, 7, 15, 30, 180], format_func=lambda x: f"{x} dias" if x < 30 else f"{x//30} meses" if x == 180 else f"{x} dias", key=f"dias_retorno_{_id}")
                if st.form_submit_button("📅 Agendar", use_container_width=True, type="primary"):
                    ...
                    clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"retorno_agendado": data_retorno}})
            ...
            with col_ac4:
                ...
                if st.form_submit_button("🚫 Não Perturbar 6m", use_container_width=True, type="secondary"):
                    ...
                    clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": {"retorno_agendado": data_retorno, "observacoes_followup": f"{obs}\n[NÃO PERTURBAR até {data_retorno}]"}})
    ...
    if selecionados:
        ...
        with col_lote1:
            if st.button("📅 Agendar retorno em 3 dias (Lote)", use_container_width=True):
                ...
                for cid in selecionados: clientes_collection.update_one({"_id": cid}, {"$set": {"retorno_agendado": data_retorno}})
        ...
        with col_lote2:
            if st.button("🚫 Não Perturbar 6 meses (Lote)", use_container_width=True):
                ...
                for cid in selecionados:
                    ...
                    clientes_collection.update_one({"_id": cid}, {"$set": {"retorno_agendado": data_retorno, "observacoes_followup": f"{obs_atual}\n[NÃO PERTURBAR até {data_retorno}]"}})
        ...
        with col_lote3:
            if st.button("❌ Remover da lista (Lote)", use_container_width=True, type="secondary"):
                for cid in selecionados: clientes_collection.update_one({"_id": cid}, {"$set": {"status_followup": "removido"}})
                st.success(f"✅ {len(selecionados)} clientes removidos da lista!")
                st.rerun()

============================================================================
✅ FUNÇÃO PRINCIPAL: render_followup (ATUALIZADA COM DELEGAÇÃO NA ABA 1)
============================================================================
def render_followup(clientes_collection):
    ...
    # 🔄 SINCRONIZAÇÃO DO MODO DELEGAÇÃO COM O BANCO
    config_banco = clientes_collection.database["configuracoes"].find_one({"tipo": "modo_delegacao"})
    if "modo_delegacao_init" not in st.session_state:
        if config_banco and config_banco.get("ativo"):
            st.session_state.modo_delegacao_ativo_sessao = config_banco.get("ativo", False)
            st.session_state.atendente_delegado_sessao = config_banco.get("atendente", "Todos os atendentes")
            st.session_state.persistir_delegacao = True
            ...
            st.session_state.atendente_delegado_sessao = "Todos os atendentes"
            st.session_state.persistir_delegacao = False
        st.session_state.modo_delegacao_init = True

    # Atualiza sessão se o banco tiver valores diferentes
    if config_banco:
        banco_ativo = config_banco.get("ativo", False)
        banco_atendente = config_banco.get("atendente", "Todos os atendentes")
        if (banco_ativo != st.session_state.get("modo_delegacao_ativo_sessao", False) or 
            banco_atendente != st.session_state.get("atendente_delegado_sessao", "Todos os atendentes")):
            st.session_state.modo_delegacao_ativo_sessao = banco_ativo
            st.session_state.atendente_delegado_sessao = banco_atendente

    # =============== TAB 1: LISTA TRADICIONAL ===============
    with tab1:
        col_busca, col_tipo = st.columns([3, 1])
        with col_tipo:
            tipo_busca = st.selectbox("Buscar por:", options=["Telefone", "Nome"], index=0, key="followup_tipo_busca")
        with col_busca:
            placeholder = "Ex: 11999999999" if tipo_busca == "Telefone" else "Ex: João Silva"
            busca_texto = st.text_input(f"🔍 Digite para buscar por {tipo_busca.lower()}:", placeholder=placeholder, key="followup_busca").strip()

        col1, col2 = st.columns([2, 2])
        with col1:
            filtro_data = st.selectbox("Filtrar por data de follow-up:", ["Todos", "Com data definida", "Sem data definida", "Vencidas", "Hoje", "Próximos 7 dias"], index=0)
        with col2:
            filtro_origem = st.multiselect("Filtrar por origem:", ["Opa Suite", "Whatsapp", "Indicação", "Loja"], default=[], key="followup_filtro_origem")
        ...
        with col_condominio[0]:
            ...
            condominios_opcoes = get_condominios_com_contagem(clientes_collection)
            opcoes_display = list(condominios_opcoes.keys())
            filtro_condominio = st.multiselect("Condomínio:", options=opcoes_display, default=[], key="followup_filtro_condominio")

        # ==========================================
        # ⚙️ PAINEL DE DELEGAÇÃO (SOMENTE ADMIN) - ABA 1
        # ==========================================
        if is_admin:
            modo_del = st.session_state.get("modo_delegacao_ativo_sessao", False)
            atendente_del = st.session_state.get("atendente_delegado_sessao", "Todos os atendentes")

            if modo_del:
                if atendente_del == "Todos os atendentes":
                    st.warning("🚨 **MODO DELEGAÇÃO ATIVO** - Todos os atendentes estão vendo TODOS os clientes!")
                else:
                    st.warning(f"🚨 **MODO DELEGAÇÃO ATIVO** - Apenas **{atendente_del}** está vendo TODOS os clientes!")

            todos_atendentes = clientes_collection.distinct("cadastrado_por", {
                "seguiu_ativacao": {"$ne": "Sim"}, "restritivo": {"$ne": "Sim"}, "status_followup": {"$ne": "removido"}
            })
            todos_atendentes = [a for a in todos_atendentes if a]
            opcoes_delegacao = ["Todos os atendentes"] + sorted(todos_atendentes)

            with st.expander("⚙️ Configurar Modo Delegação", expanded=not modo_del):
                col_d1, col_d2, col_d3 = st.columns([1, 2, 2])
                with col_d1:
                    novo_modo = st.toggle("🔄 Ativar", value=modo_del, help="Permite delegar visão da carteira completa.", key="toggle_delegacao_tab1")
                    st.session_state.modo_delegacao_ativo_sessao = novo_modo
                with col_d2:
                    if novo_modo:
                        novo_atendente = st.selectbox("👤 Quem recebe todos os clientes:", options=opcoes_delegacao, 
                                                      index=opcoes_delegacao.index(atendente_del) if atendente_del in opcoes_delegacao else 0, 
                                                      key="select_atendente_tab1")
                        st.session_state.atendente_delegado_sessao = novo_atendente
                with col_d3:
                    persistir = st.checkbox("💾 Salvar no banco", value=st.session_state.get("persistir_delegacao", False), key="check_persistir_tab1")
                    st.session_state.persistir_delegacao = persistir

                col_btn1, col_btn2 = st.columns(2)
                with col_btn1:
                    if st.button("💾 Aplicar", use_container_width=True, type="primary", key="aplicar_deleg_tab1"):
                        if persistir:
                            clientes_collection.database["configuracoes"].update_one(
                                {"tipo": "modo_delegacao"},
                                {"$set": {"tipo": "modo_delegacao", "ativo": novo_modo, "atendente": novo_atendente, 
                                          "ativado_por": usuario_atual, "data_ativacao": datetime.now(timezone.utc).isoformat()}},
                                upsert=True)
                            st.success("✅ Salvo no banco!")
                        else:
                            clientes_collection.database["configuracoes"].delete_one({"tipo": "modo_delegacao"})
                            st.info("ℹ️ Aplicado apenas para esta sessão.")
                        st.rerun()
                with col_btn2:
                    if st.button("🗑️ Limpar do Banco", use_container_width=True, type="secondary", key="limpar_deleg_tab1"):
                        clientes_collection.database["configuracoes"].delete_one({"tipo": "modo_delegacao"})
                        st.session_state.modo_delegacao_ativo_sessao = False
                        st.session_state.atendente_delegado_sessao = "Todos os atendentes"
        ...
        # 🔑 FILTRO POR USUÁRIO / DELEGAÇÃO (SUBSTITUI O FILTRO RÍGIDO)
        usuario_pode_ver_todos = is_admin or (
            st.session_state.get("modo_delegacao_ativo_sessao", False) and (
                st.session_state.get("atendente_delegado_sessao", "Todos os atendentes") == "Todos os atendentes" or
                st.session_state.get("atendente_delegado_sessao", "Todos os atendentes") == usuario_atual
            )
        )

        if not usuario_pode_ver_todos and usuario_atual:
        ...
        if not clientes_followup:
            ...
            for cliente in clientes_followup:
                exibir_cliente_detalhe(cliente, clientes_collection, key_suffix="tab1_")
    ...
    with tab2:
        ...
        st.caption("🎨 Legendas: ⚪ Sem follow-up | 🟢 ≤2 | 🟡 3–5 | 🟠 6–10 | 🔴 ≥11 | ❗ Dias vencidos com follow-up pendente")
        ...
        for semana in cal:
            ...
            for i, dia_num in enumerate(semana):
                if dia_num == 0: cols[i].write(" ")
                    ...
                    else: media_touches = 0

                    if qtd == 0: cor, texto = "#f8f9fa", str(dia_num)
                    elif media_touches <= 2: cor, texto = "#d4edda", f"{dia_num}<br/>({qtd})<br/><small>avg: {media_touches:.1f}</small>"
                    elif media_touches <= 5: cor, texto = "#fff3cd", f"{dia_num}<br/>({qtd})<br/><small>avg: {media_touches:.1f}</small>"
                    elif media_touches <= 10: cor, texto = "#ffeacc", f"{dia_num}<br/>({qtd})<br/><small>avg: {media_touches:.1f}</small>"
                    else: cor, texto = "#f8d7da", f"{dia_num}<br/>({qtd})<br/><small>avg: {media_touches:.1f}</small>"

                    borda, icone = " ", " "
                    if data < hoje_date and qtd > 0: borda, icone = "border: 2px solid #e74c3c; ", "❗ "
                    estilo = f"background-color:{cor}; padding:10px; border-radius:6px; text-align:center; font-weight:bold; font-size:0.9em; {borda}"
        ...
        st.markdown("---")
        data_selecionada = st.date_input("Selecione um dia para ver os follow-ups:", value=datetime.now(timezone.utc), min_value=datetime(2020, 1, 1), key="followup_seleciona_dia")
        ...
        if clientes_do_dia:
            ...
            for cliente in clientes_do_dia:
                ...
                if condominio:
                    ...
                    if bloco or apto:
                        unidade = []
                        if bloco: unidade.append(f"Bloco {bloco}")
                        if apto: unidade.append(f"Apto {apto}")
                        condominio_info += f" ({' / '.join(unidade)})"
                texto_export += (f"📞 {nome} (toques: {touch_ct}){condominio_info}\n📱 {tel}\n🎯 Origem: {origem}\n📋 Plano: {plano}\n👤 Cadastrado por: {cad_por}\n📝 Obs: {obs}\n---\n")

            st.download_button(label="📋 Copiar todos os follow-ups do dia", data=texto_export, file_name=f"followups_{data_selecionada.strftime('%Y-%m-%d')}.txt", mime="text/plain")
            for cliente in clientes_do_dia:
                exibir_cliente_detalhe(cliente, clientes_collection, key_suffix="calendario_")
        else:

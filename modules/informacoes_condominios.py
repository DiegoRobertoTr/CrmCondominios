...
def render_editor_informacoes():
    """Editor completo de informações do condomínio"""
    st.subheader("✏️ Editor de Informações Detalhadas")
    ...
    if 'cond_info_edit' in st.session_state:
        for idx, cond in enumerate(condominios):
            if str(cond['_id']) == st.session_state['cond_info_edit']:
                default_index = idx
                break
    ...
    if cond_selecionado:
        ...
        if cond_atual:
            render_formulario_informacoes(cond_atual, collection)
            ...
            with st.container(border=True):
                ...
                with col1:
                    doacao["item"] = st.text_input(
                        ...
                        key=f"item_doacao_{cond['_id']}_{idx}"
                    )
                ...
                with col2:
                    doacao["quantidade"] = st.number_input(
                        ...
                        key=f"qtd_doacao_{cond['_id']}_{idx}"
                    )
                ...
                with col3:
                    doacao["valor"] = st.number_input(
                        ...
                        step=0.01,
                        key=f"valor_doacao_{cond['_id']}_{idx}"
                    )
                ...
                with col5:
                    doacao["forma_pagamento"] = st.selectbox(
                        ...
                        key=f"forma_doacao_{cond['_id']}_{idx}"
                    )
            ...
            with st.container(border=True):
                ...
                with col1:
                    contato["nome"] = st.text_input(
                        f"Contato {idx+1} - Nome",
                        value=contato.get("nome", ""),
                        placeholder="Nome completo",
                        key=f"nome_contato_{cond['_id']}_{idx}"
                    )
                ...
                with col2:
                    contato["telefone"] = st.text_input(
                        ...
                        key=f"tel_contato_{cond['_id']}_{idx}"
                    )
                ...
                with col3:
                    contato["funcao"] = st.selectbox(
                        ...
                        key=f"funcao_contato_{cond['_id']}_{idx}"
                    )

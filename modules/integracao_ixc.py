# modules/integracao_ixc.py - VERSÃO FINAL ATUALIZADA
import requests
import base64
import json
...
# ============================================================================
# CONSTRUÇÃO DO PAYLOAD PARA IXC (LÓGICA CONDICIONAL ATUALIZADA)
# ============================================================================
def construir_payload_ixc(cliente_data: Dict, config: Dict) -> Tuple[Dict, Optional[str]]:
    """Constrói payload seguro para IXC com lógica condicional para condomínio."""
    ...
    # 3. Buscar ID do Condomínio no IXC
    ...
    # 4. Campos base (sempre obrigatórios)
    ...
    # 6. Validação condicional de endereço
    endereco_payload = {}
    if id_condominio_ixc:
        # ✅ SE HÁ CONDOMÍNIO: NÃO VALIDA NEM ENVIA CAMPOS DE ENDEREÇO.
        # O IXC os preencherá automaticamente com os dados do condomínio.
        pass
    else:
        # ✅ SEM CONDOMÍNIO: VALIDA E INCLUI ENDEREÇO MANUAL
        cep_raw = safe(cliente_data.get("cep"))
        cep = "".join(filter(str.isdigit, cep_raw))
        if cep and len(cep) != 8:
            return {}, f"CEP inválido: '{cep_raw}'. Deve conter 8 dígitos."
        ...
        obrigatorios_end = {"cidade": cidade, "uf": uf, "endereco": endereco, "numero": numero, "bairro": bairro, "cep": cep}
        faltando_end = [k for k, v in obrigatorios_end.items() if not v]
        if faltando_end:
            return {}, f"Campos obrigatórios de endereço ausentes: {', '.join(faltando_end)}"
    ...
    # 7. Validação de campos obrigatórios básicos
    obrigatorios_basicos = {"razao": nome_completo, "cnpj_cpf": cpf, "fone": fone, "email": email}
    faltando = [k for k, v in obrigatorios_basicos.items() if not v]
    if faltando:
        return {}, f"Campos obrigatórios ausentes: {', '.join(faltando)}"

    # 8. Montagem do payload base
    payload = {
        "ativo": "S", "id_tipo_cliente": config.get("id_tipo_cliente", "03"),
        "tipo_cliente_scm": config.get("tipo_cliente_scm", "01"),
        "filial_id": config.get("filial_id", "1"), "filtra_filial": "S",
        "tipo_pessoa": "F", "razao": nome_completo, "nome_social": nome_completo,
        "fantasia": nome_completo, "cnpj_cpf": cpf, "ie_identidade": rg,
        "data_nascimento": data_nasc_formatada, "email": email,
        "telefone_celular": celular, "whatsapp": celular, "fone": fone,
        "acesso_automatico_central": "P", "alterar_senha_primeiro_acesso": "S",
        "hotsite_acesso": "2", "senha_hotsite_md5": "N", "hotsite_email": email,
        "senha": "123456", "iss_classificacao_padrao": "99",
        "participa_cobranca": "S", "participa_pre_cobranca": "S",
        "cob_envia_email": "S", "cob_envia_sms": "S", "contribuinte_icms": "N",
        "nacionalidade": "Brasileiro", "status_prospeccao": "C", "tipo_assinante": "3",
        "obs": obs
    }

    # 9. Aplica lógica condicional
    if id_condominio_ixc:
        payload["id_condominio"] = id_condominio_ixc
    else:
        payload.update(endereco_payload)

    # Remove campos vazios/nulos para evitar validação indevida no IXC

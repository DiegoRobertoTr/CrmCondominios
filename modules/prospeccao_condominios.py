...
def update_records_batch_vectorized(db, df_original, df_editado, colunas_para_comparar):
    ...
    try:
        ...
        mask_precisa_prioridade = pd.Series([False] * len(df_editado_alterado), index=df_editado_alterado.index)
...
def render_prospeccao_condominios():
    ...
    with col2:
        ...
        if st.button("🗑️ Limpar Dados", type="secondary", use_container_width=True):
            if st.session_state.get("confirm_delete_prospeccao"):
                ...
                if "df_prospeccao_cached" in st.session_state:
                    del st.session_state["df_prospeccao_cached"]
                # ✅ CORREÇÃO: limpar flag de arquivo importado ao limpar dados
    ...
    # ==================== IMPORTAÇÃO DA PLANILHA - CORRIGIDA ====================
    if uploaded_file is not None:

        # ✅ CORREÇÃO PRINCIPAL: Gerar chave única para o arquivo atual
        # Combina nome + tamanho para identificar unicamente o arquivo
        file_key = f"{uploaded_file.name}_{uploaded_file.size}"

        # ✅ Se este arquivo já foi importado com sucesso nesta sessão, não reimportar
        if st.session_state.get("last_imported_file") == file_key:
            # Arquivo já processado — apenas garantir carregamento do cache
            if "df_prospeccao_cached" not in st.session_state:
                st.session_state["reload_prospeccao"] = True
        else:
            # Novo arquivo: processar importação
            ...
            try:
                ...
                # ✅ CORREÇÃO: criar mapeamento correto usando lowercase das colunas atuais
                rename_map = {}
                cols_lower_map = {c.lower(): c for c in df_prospeccao.columns}
                for key_lower, new_name in col_mapping.items():
                    if key_lower in cols_lower_map:
                        original_col = cols_lower_map[key_lower]
                ...
                if save_prospeccao_data(db, df_prospeccao, metadata):
                    ...
                    # ✅ CORREÇÃO PRINCIPAL: Marcar arquivo como importado ANTES do rerun
                    # Isso evita que o rerun reprocesse o mesmo arquivo novamente
    ...
    with tab_update:
        ...
        if not df_filtered.empty:
            ...
            if st.button("💾 Salvar Alterações Selecionadas", type="primary", key="btn_save_updates"):
                ...
                with st.spinner('🔄 Processando alterações de forma vetorizada...'):

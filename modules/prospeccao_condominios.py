...
def classificar_fase_vetorizado(fases_series):
    """Versão vetorizada para classificar fases - MUITO MAIS RÁPIDO"""
...
def calcular_dias_para_entrega_vetorizado(previsao_series):
    ...
    if previsao_series is None or len(previsao_series) == 0:
        return pd.Series([None] * len(previsao_series))
...
def update_records_batch_vectorized(db, df_original, df_editado, colunas_para_comparar):
    try:
        ...
        # Recalcular campos derivados com segurança
        if 'ESTÁGIO' in df_editado_alterado.columns and 'ESTÁGIO' in colunas_para_comparar:
        ...
        if 'VIABILIDADE' in df_editado_alterado.columns and 'VIABILIDADE' in colunas_para_comparar:
        ...
        if 'PREVISAO_ENTREGA' in df_editado_alterado.columns and 'PREVISAO_ENTREGA' in colunas_para_comparar:
        ...
        for col in colunas_que_afetam_prioridade:
            if col in colunas_para_comparar and col in df_editado_alterado.columns and col in df_original_alterado.columns:
        ...
        for idx, record_id in enumerate(ids_alterados):
            ...
            # Adicionar campos recalculados se existirem
            if 'FASE_CLASSIFICADA' in df_editado_alterado.columns:
                if 'FASE_CLASSIFICADA' in updates or str(df_original_alterado['FASE_CLASSIFICADA'].iloc[idx]) != str(df_editado_alterado['FASE_CLASSIFICADA'].iloc[idx]):
            ...
            if 'PREVISAO_ENTREGA' in df_editado_alterado.columns:
                 if 'PREVISAO_ENTREGA' in updates or str(df_original_alterado['PREVISAO_ENTREGA'].iloc[idx]) != str(df_editado_alterado['PREVISAO_ENTREGA'].iloc[idx]):
                    updates['PREVISAO_ENTREGA'] = df_editado_alterado['PREVISAO_ENTREGA'].iloc[idx]
            
            if 'DIAS_RESTANTES' in df_editado_alterado.columns:
                 if 'DIAS_RESTANTES' in updates or str(df_original_alterado['DIAS_RESTANTES'].iloc[idx]) != str(df_editado_alterado['DIAS_RESTANTES'].iloc[idx]):
                    updates['DIAS_RESTANTES'] = df_editado_alterado['DIAS_RESTANTES'].iloc[idx]
            
            if 'PRIORIDADE' in df_editado_alterado.columns:
                 if 'PRIORIDADE' in updates or str(df_original_alterado['PRIORIDADE'].iloc[idx]) != str(df_editado_alterado['PRIORIDADE'].iloc[idx]):
...
def callback_confirmar_limpeza_total():
    db = init_mongo()
...
def callback_cancelar_limpeza_total():
    st.session_state.confirmar_limpeza_total = False
    st.rerun()

def callback_confirmar_manter_ultimo():
    db = init_mongo()
    collection = db["prospeccao_condominios"]
    meta_collection = db["prospeccao_meta"]
    ultimo_meta = meta_collection.find_one(sort=[("timestamp", -1)])
...
def callback_cancelar_manter_ultimo():
    st.session_state.confirmar_manter_ultimo = False
    st.rerun()

def callback_salvar_edicoes(edited_df, df_original_edit, cols_existing):
    db = init_mongo()
    with st.spinner(f' Processando alterações de forma vetorizada...'):
        modified_count = update_records_batch_vectorized(db, df_original_edit, edited_df, cols_existing)
        if modified_count > 0:
...
def render_prospeccao_condominios():
    ...
    if "reload_prospeccao" not in st.session_state:
        st.session_state.reload_prospeccao = False
    ...
    with st.expander("⚠️ FERRAMENTAS DE MANUTENÇÃO (Administrador)", expanded=False):
        ...
        with col_limpeza2:
            st.button("🗑️ LIMPAR TODOS OS DADOS (URGENTE)", on_click=callback_limpar_tudo, key="btn_limpar_tudo")
        
        with col_limpeza3:
            st.button("🧹 Manter Apenas Último Lote", on_click=callback_manter_ultimo, key="btn_manter_ultimo")
        ...
        if st.session_state.get('confirmar_limpeza_total', False):
            ...
            with col_confirm1:
                st.button("✅ SIM, REMOVER TUDO", on_click=callback_confirmar_limpeza_total, key="confirmar_sim")
            with col_confirm2:
                st.button("❌ Cancelar", on_click=callback_cancelar_limpeza_total, key="confirmar_nao")
        ...
        if st.session_state.get('confirmar_manter_ultimo', False):
            ...
            with col_confirm3:
                st.button("✅ Sim, manter apenas último", on_click=callback_confirmar_manter_ultimo, key="manter_sim")
            with col_confirm4:
                st.button(" Cancelar", on_click=callback_cancelar_manter_ultimo, key="manter_nao")
    ...
    with col1:
        uploaded_file = st.file_uploader(
            ...
            on_change=callback_importar,
            key="file_uploader_key"
        )

    with col2:
        st.button(" Recarregar Últimos", on_click=callback_recargar, type="primary", use_container_width=True, key="btn_recargar")
        
        if st.button("🗑️ Limpar Dados", type="secondary", use_container_width=True, key="btn_limpar_dados"):
             # Lógica simplificada: se já confirmado, limpa. Se não, pede confirmação.
            if st.session_state.get("confirm_delete_prospeccao"):
                callback_limpar_dados()
            else:
                st.session_state["confirm_delete_prospeccao"] = True
                st.rerun()

    # ==================== IMPORTAÇÃO DA PLANILHA OTIMIZADA ====================
    if st.session_state.trigger_import and uploaded_file is not None:
        ...
        try:
            ...
            # Correção da lógica de cabeçalho duplicado
            if len(df_prospeccao) > 0:
                primeira_linha = df_prospeccao.iloc[0].astype(str).str.lower().str.strip()
                col_names_lower = [str(c).lower().strip() for c in df_prospeccao.columns]
                
                # Verifica se TODOS os valores da primeira linha estão nas colunas OU são vazios/nan
                # Ignora 'nan' string que vem de células vazias
                is_header_duplicate = all(
                    (val in col_names_lower or val == '' or val == 'nan') 
                    for val in primeira_linha
                )
                
                # Só pula se realmente parecer um cabeçalho duplicado (ex: mais de 80% das colunas batem)
                matches = sum(1 for val in primeira_linha if val in col_names_lower)
                if is_header_duplicate and matches > len(df_prospeccao.columns) * 0.5:
            ...
            if save_prospeccao_data(db, df_prospeccao, metadata):
                ...
                # Resetar flag de trigger para evitar loop
                st.session_state.trigger_import = False
                ...
                # Forçar reload na próxima iteração natural
                st.session_state.reload_prospeccao = True
                st.rerun()
    ...
    elif st.session_state.reload_prospeccao or "df_prospeccao_cached" not in st.session_state:
    ...
    # Limpar flag de reload após uso
    ...
    if df_prospeccao is not None and not df_prospeccao.empty:
        ...
        with tab_update:
            ...
            if not df_filtered.empty:
                ...
                if st.button("💾 Salvar Alterações Selecionadas", type="primary", key="btn_save_updates"):
                    ...
                    # Chama a função de callback diretamente aqui para garantir contexto
                    callback_salvar_edicoes(edited_df, df_original_edit, cols_existing)
            else:
        ...
        # ==================== DASHBOARD PRINCIPAL ====================
        # (O restante do código das abas de dashboard permanece igual ao anterior, pois não causava loop)
        # Para economizar espaço, mantenha a lógica das abas tab_dash1 a tab_dash5 idêntica à versão anterior,
        # garantindo apenas que não haja chamadas diretas de st.rerun() dentro de loops ou condições de renderização.
        # Abaixo, um exemplo simplificado de como estruturar para evitar problemas:
        
        with tab_dash1:
            st.header("📊 Análise por Construtora")
            df_construtoras = analisar_por_construtora(df_prospeccao)
            if not df_construtoras.empty:
                # ... (mantenha o código de gráficos e tabelas aqui) ...
                ...
                construtoras_sel = st.multiselect("Filtrar Construtoras", options=construtoras_disp, default=default_construtoras, key="construtoras_filter")
                if construtoras_sel:
                    df_construtoras_filt = df_construtoras[df_construtoras["CONSTRUTORA"].isin(construtoras_sel)]
                    # Gráficos...
                    st.dataframe(df_construtoras_filt, use_container_width=True)
            else:
                st.warning("️ Dados insuficientes para análise por construtora")
        
        # Repita a estrutura segura para as outras abas (tab_dash2, tab_dash3, etc.)
        # Certifique-se de que nenhum st.rerun() seja chamado fora de callbacks ou ações de botão explícitas.
        
        with tab_dash5:
             st.header("📋 Lista Completa de Projetos")
             # ... filtros e dataframe ...
             st.dataframe(df_lista, use_container_width=True)
             # Botão de exportação seguro
             excel_buffer = exportar_prospeccao_excel(df_filt, analisar_por_construtora(df_filt), analisar_por_zona(df_filt))
             st.download_button("📥 Exportar Lista Completa", excel_buffer, f"prospeccao_completa_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

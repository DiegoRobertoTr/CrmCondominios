...
def converter_dataframe_dates_otimizado(df, colunas_alvo=None):
    """
    ✅ OTIMIZAÇÃO: Processa todas as colunas de data em uma única passagem
    """
    if df.empty:
        return df
...
def save_prospeccao_data_otimizado(db, df_prospeccao, metadata):
    ...
    # Identifica colunas de data para conversão em lote
    ...
    docs = []
    timestamp_atual = datetime.now().replace(tzinfo=None)
    
    # Converte DataFrame para lista de dicionários de uma vez
...
def load_latest_prospeccao(db):
    ...
    # Mantém o _id e converte para string para o Streamlit editar
    ...
    try:
        ...
        for col in date_cols:
            if clean_updates[col] is not None:
                if isinstance(clean_updates[col], (pd.Timestamp, datetime)):
                    if hasattr(clean_updates[col], 'tzinfo') and clean_updates[col].tzinfo is not None:
                        clean_updates[col] = clean_updates[col].replace(tzinfo=None)
...
def insert_new_record(db, new_data):
    ...
    try:
        ...
        if "ESTÁGIO" in doc:
            ...
            doc["FASE_CLASSIFICADA"] = "💼 Em Negociação"
...
def classificar_fase(fase_str):
    ...
    # ✅ OTIMIZAÇÃO: Usa dicionário para lookup mais rápido
    fase_lower = str(fase_str).lower().strip()
    
    # Mapeamento direto para performance
    mapeamento_fases = {
        "em negociação": "💼 Em Negociação",
        "negociando": "💼 Em Negociação",
        "tratativa avançada": "💼 Em Negociação",
        "proposta": "💼 Em Negociação",
        "estudo": "💼 Em Negociação",
        "análise": "💼 Em Negociação",
        "entramos": "✅ Entramos",
        "entrada confirmada": "✅ Entramos",
        "projeto aceito": "✅ Entramos",
        "ganhamos": "✅ Entramos",
        "pronto": "✅ Pronto",
        "entregue": "✅ Pronto",
        "finalizado": "✅ Pronto",
        ...
        "fase final": "🏁 Final de Obra",
        "acabamento": "🏁 Final de Obra",
        "estágio final": "🏁 Final de Obra",
        "estagio final": "🏁 Final de Obra",
        "intermediário": "🔨 Intermediário",
        "intermediario": "🔨 Intermediário",
        "intermed.": "🔨 Intermediário",
        "avançado": "🔨 Intermediário",
        "avancado": "🔨 Intermediário",
        "início de obra": "🚧 Início de Obra",
        "inicio de obra": "🚧 Início de Obra",
        "inicial": "🚧 Início de Obra",
        "fundação": "🚧 Início de Obra",
        "estrutura": "🚧 Início de Obra",
        "obra em andamento": "🚧 Início de Obra",
        "lançamento": "📢 Lançamento",
        "lancamento": "📢 Lançamento",
        "vendas": "📢 Lançamento",
        "grupo em formação": "📢 Lançamento",
        "futuro": "📅 Futuro Lançamento",
        "planejado": "📅 Futuro Lançamento",
        "terreno": "📅 Futuro Lançamento",
        "futuro lançamento": "📅 Futuro Lançamento",
        "não entramos": "❌ Não Entramos",
        "perdido": "❌ Não Entramos",
        "embargado": "❌ Não Entramos",
        "sem viabilidade": "❌ Não Entramos",
        "não autorizado": "❌ Não Entramos"
    }
    
    # Busca direta no dicionário
    for chave, valor in mapeamento_fases.items():
        if chave in fase_lower:
            return valor
    
    # Fallback
    return "💼 Em Negociação"

def extrair_previsao_entrega_vectorizado(serie):
    """
    ✅ OTIMIZAÇÃO: Extrai datas de uma coluna inteira de uma vez
    """
    def extrair_individual(valor):
        if pd.isna(valor):
            return None
        valor_str = str(valor)
        ...
        for padrao in padroes_data:
            match = re.search(padrao, valor_str, re.IGNORECASE)
            if match:
                try:
    ...
    return serie.apply(extrair_individual)
...
def calcular_dias_para_entrega_vectorizado(serie):
    """
    ✅ OTIMIZAÇÃO: Calcula dias para toda a coluna de uma vez
    """
    hoje = datetime.now().replace(tzinfo=None)
    return (serie - pd.Timestamp(hoje)).dt.days
...
def analisar_por_zona(df_prospeccao):
    ...
    zona_stats = df_prospeccao.groupby(col_zona).agg(
        ...
        projetos_pronto=("FASE_CLASSIFICADA", lambda x: (x == "✅ Pronto").sum()),
        oportunidades=("FASE_CLASSIFICADA", lambda x: x.isin(["📢 Lançamento", "📅 Futuro Lançamento", "🔨 Intermediário", "🚧 Início de Obra", "✅ Entramos", "💼 Em Negociação"]).sum())
    ).reset_index()
...
def timeline_entregas(df_prospeccao):
    ...
    df_timeline["DIAS_RESTANTES"] = calcular_dias_para_entrega_vectorizado(df_timeline["PREVISAO_ENTREGA"])
...
def calcular_prioridade_vectorizado(df):
    """
    ✅ OTIMIZAÇÃO: Calcula prioridade para todo o DataFrame de uma vez
    """
    def calcular_linha(row):
        ...
        elif fase == "💼 Em Negociação":
            return "🟠 Alta Prioridade"
        elif fase in ["✅ Pronto", "🏁 Final de Obra"]:
...
def exportar_prospeccao_excel(df_prospeccao, df_construtoras, df_zonas):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        fases_map = {
            '✅ Entramos': '00_Entremos_Destaque',
            '💼 Em Negociação': '01_Em_Negociacao',
            '📢 Lançamento': '02_Lancamento',
            '🚧 Início de Obra': '03_Inicio_Obra',
            '🔨 Intermediário': '04_Intermediario',
            '🏁 Final de Obra': '05_Final_Obra',
            '✅ Pronto': '06_Pronto',
            '📅 Futuro Lançamento': '07_Futuro_Lancamento',
            '❌ Não Entramos': '08_Nao_Entramos'
        }
        ...
        if not df_construtoras.empty:
            df_construtoras.to_excel(writer, sheet_name='09_Por_Construtora', index=False)
        
        if not df_zonas.empty:
            df_zonas.to_excel(writer, sheet_name='10_Por_Regiao', index=False)
...
def render_prospeccao_condominios():
    ...
    if uploaded_file:
        ...
        try:
            ...
            status_text.text("🔄 Processando estrutura...")
            ...
            if len(df_prospeccao) > 0:
                ...
                status_text.text("📅 Processando datas...")
                progress_bar.progress(80)
                if "VIABILIDADE" in df_prospeccao.columns:
                    # ✅ OTIMIZAÇÃO: Processa coluna inteira de uma vez
                    df_prospeccao["PREVISAO_ENTREGA"] = extrair_previsao_entrega_vectorizado(df_prospeccao["VIABILIDADE"])
                
                if "Previsão de Entrega" in df_prospeccao.columns:
                    df_prospeccao["PREVISAO_ENTREGA_2"] = extrair_previsao_entrega_vectorizado(df_prospeccao["Previsão de Entrega"])
                ...
                if "PREVISAO_ENTREGA" in df_prospeccao.columns:
                    df_prospeccao["DIAS_RESTANTES"] = calcular_dias_para_entrega_vectorizado(df_prospeccao["PREVISAO_ENTREGA"])
                
                df_prospeccao["PRIORIDADE"] = calcular_prioridade_vectorizado(df_prospeccao)
                ...
                # ✅ OTIMIZAÇÃO CRÍTICA: Insert em batch
                if save_prospeccao_data_otimizado(db, df_prospeccao, metadata):
            ...
            if not df_filtered.empty:
                ...
                if st.button("💾 Salvar Alterações Selecionadas", type="primary"):
                    ...
                    if success_count > 0:
                        ...
                        st.rerun()
                    if error_count > 0:
                        st.error(f"⚠️ {error_count} registros falharam ao atualizar.")
            else:
    ...
    if df_prospeccao is not None and not df_prospeccao.empty:
        ...
        with tab_dash1:
            ...
            if not df_construtoras.empty:
                ...
                if construtoras_sel:
                    ...
                    fases_cols = ["projetos_entramos", "projetos_negociacao", "projetos_pronto", "projetos_final_obra", "projetos_intermediario", "projetos_inicio_obra", "projetos_lancamento", "projetos_futuro"]
                    fases_labels = ["✅ Entramos", "💼 Negociação", "✅ Pronto", "🏁 Final", "🔨 Intermed.", "🚧 Início", "📢 Lançam.", "📅 Futuro"]
        ...
        with tab_dash4:
            ...
            if "PRIORIDADE" in df_prospeccao.columns:
                ...
                with col_pri1:
                    fig_pri = px.pie(values=df_prospeccao["PRIORIDADE"].value_counts().values, names=df_prospeccao["PRIORIDADE"].value_counts().index, title="Distribuição de Prioridades", color_discrete_map={
                        "🔴 Urgente": "#e74c3c", 
                        "🟠 Alta": "#e67e22", 
                ...
                with col_pri2:
                    ...
                    valid_defaults = [p for p in ["🔴 Urgente", "🟢 Ação Imediata", "🟠 Alta Prioridade"] if p in prioridades_disp]
        ...
        - A fase **💼 Em Negociação** substituiu "Em Tratativa" para maior clareza comercial.
        - A fase **✅ Entramos** destaca projetos confirmados com alta prioridade.

...
def _leads_por_vendedora_followup(collection, inicio, fim, modo_delegacao_ativo=False, atendente_delegado=None):
    ...
    # Filtro de período (data_cadastro)
    if inicio and fim:
    ...
    # Adicionar atendentes que não têm leads (mostrar como zero)
    if modo_delegacao_ativo and atendente_delegado:
        # Se modo delegação ativo, mostrar apenas o(s) atendente(s) delegado(s)
        ...
        # Buscar todos os atendentes com leads
        todos_atendentes = collection.distinct("cadastrado_por", query)
        todos_atendentes = [a for a in todos_atendentes if a]
    
    # Criar dicionário com todos os atendentes
...
def render_relatorios_followup(clientes_collection):
    ...
    with col1:
        # Gráfico de barras - Leads por vendedora
    ...
    with col2:
        # Gráfico de pizza - Distribuição por vendedora
        df_pizza = df[df["total_leads"] > 0].copy()
        
        if not df_pizza.empty:
            # Limitar a 10 itens para não poluir o gráfico
            if len(df_pizza) > 10:
    ...
    # ============ DETALHES POR VENDEDORA (EXPANDER) ============
    st.markdown("---")
...
def render_relatorios(clientes_collection):
    ...
    # Criar abas com base no perfil do usuário
    if is_admin or is_diretoria:
        ...
        # ==================== ABA 2: FOLLOW-UP LEADS POR VENDEDORA ====================
        with tabs[1]:

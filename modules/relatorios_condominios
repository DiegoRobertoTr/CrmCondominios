...
def save_condominio_data(db, df_clientes, df_condominios, metadata):
    ...
    # Limpa dados anteriores (opcional - pode ser configurável)
    ...
    # Salva metadados dos condomínios
...
def load_latest_data(db):
    ...
    # Remove campos internos do pandas
...
def analisar_inadimplencia(df_clientes, df_condominios):
    ...
    # Normaliza coluna de atraso
    df_clientes["atraso_bin"] = df_clientes["FINANCEIRO EM ATRASO"].apply(
        lambda x: "Em Atraso" if pd.notna(x) and str(x).strip().lower() not in ["00/00/0000", "", "0", "nan"] 
        else "Em Dia"
    )
    ...
    # Merge com info dos condomínios
...
def correlacao_concorrencia(df_penetracao, df_condominios):
    ...
    if "Principal Concorrente" in df_penetracao.columns:
        ...
        # Calcula penetração média ponderada
...
def calcular_receita_potencial(df_penetracao, ticket_medio=89.99):
    ...
    # Gap de oportunidade
    df["gap_receita"] = df["receita_potencial"] / df["receita_atual"] * 100
...
def render_relatorios_condominios():
    ...
    # Prioridade: 1) Upload novo, 2) Dados persistidos, 3) Vazio
    ...
    # ==================== ABA 1: PENETRAÇÃO POR CONDOMÍNIO ====================
    st.markdown("---")
    ...
    with tab1:
        ...
        fig = px.bar(
            ...
            labels={"taxa_penetracao": "Penetração (%)", "Condomínio": ""},
            color_discrete_map={
                "🟢 Dominado": "#2ecc71",
        ...
        with st.expander("📋 Ver Tabela Completa"):
            ...
            st.download_button(
                "📥 Exportar CSV",
                data=csv,
                file_name=f"penetracao_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
    ...
    with tab2:
        ...
        st.dataframe(
            df_prioridade.sort_values("receita_potencial", ascending=False).head(20),
            use_container_width=True,
            column_config={
                "receita_potencial": st.column_config.NumberColumn("Potencial (R$)", format="R$ %.2f")
            }
        )
    ...
    with tab3:
        ...
        df_merge = df_penetracao.merge(
            df_inadimplencia[["ID", "taxa_inadimplencia"]],
            on="ID",
            how="left"
        )
        ...
        fig = px.scatter(
            ...
            title="Penetração vs Inadimplência (tamanho = nº de apartamentos)",
            labels={
                "taxa_penetracao": "Penetração (%)",
    ...
    with tab4:
        ...
        # Análise de motivos (se disponível)
    ...
    with tab5:
        ...
        if not df_concorrencia.empty:
            ...
            fig = px.bar(
                ...
                labels={
                    "Principal Concorrente": "Concorrente",
                    "penetracao_ponderada": "Penetração Média (%)"
                }
            )
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)
            ...
            if "penetracao_ponderada_mean" in df_concorrencia.columns:
                ...
                with col1:
                    st.success(f"""
                    **✅ Melhor desempenho:** vs {melhor['Principal Concorrente']}
                    - Penetração média: {melhor['penetracao_ponderada']:.1f}%
                    - **Estratégia:** Replicar abordagem em outros mercados
                    """)
                
                with col2:
                    st.error(f"""
                    **⚠️ Desafio:** vs {pior['Principal Concorrente']}
                    - Penetração média: {pior['penetracao_ponderada']:.1f}%
                    - **Estratégia:** Revisar proposta de valor / preços / atendimento
                    """)
        else:
    ...
    # Scatter plot com clusterização visual
    fig_mapa = px.scatter(
        df_penetracao,
        x="Apartamentos",
        y="taxa_penetracao",
        ...
        labels={
            "Apartamentos": "Total de Apartamentos",
            "taxa_penetracao": "Taxa de Penetração (%)"
        },
        color_discrete_map={
            "🟢 Dominado": "#2ecc71",
            "🟡 Em Crescimento": "#f39c12",

...
def copiar_para_area_de_transferencia(texto, botao_key):
    ...
    components.html(
        ...
        <button onclick="copyToClipboard_{botao_key}()" style="padding: 4px 8px; cursor: pointer;">📋 Copiar</button>
        """,
        height=40,
    )
...
def carregar_condominios():
    """Carrega lista de condomínios do MongoDB."""
    try:
        # Tenta acessar a collection condominios na mesma database
        if "condominios_collection" not in st.session_state:
            db = st.session_state["clientes_collection"].database
            st.session_state["condominios_collection"] = db["condominios"]
        
        condominios = st.session_state["condominios_collection"].find({}, {"_id": 0}).sort("Condomínio", 1)
        return list(condominios)
    except Exception as e:
        # st.warning(f"⚠️ Não foi possível carregar a lista de condomínios: {e}")
        return []

def verificar_duplicidade():
    """Verifica duplicidade de CPF/Celular em tempo real. Retorna (cliente, campo_duplicado)"""
    cpf_raw = st.session_state.get(f"campo_cpf_{st.session_state['form_key']}", "").strip()
    celular_bruto = st.session_state.get(f"campo_celular_principal_{st.session_state['form_key']}", "").strip()
    ...
    if query:
        ...
        if cliente:
            if cpf and cliente.get("cpf") == cpf:
                return cliente, "cpf"
...
def render_campos_restritivos(key_suffix, valor_restritivo, cliente=None):
    if valor_restritivo == "Sim":
        ...
        with col2:
            ...
            ano_recente = st.selectbox(
                "Qual ano mais recente?",
                options=list(range(2020, ano_atual + 1)),
...
# ============================================================================
# EXPANDER PARA VISUALIZAR/EDITAR CADASTRO COMPLETO - ATUALIZADO COM CONDOMÍNIOS
# ============================================================================
    ...
    with st.expander("📋 Visualizar / Editar Cadastro Completo", expanded=True):
        ...
        with st.container(border=True):
            ...
            with col_int:
                ...
                ja_possui_internet = st.selectbox("Já Possui Internet?",
                    OPCOES_INTERNET,
            ...
            # VERIFICA SE JÁ EXISTE AGENDAMENTO VÁLIDO
        ...
        with st.form("form_editar_cadastro"):
            ...
            if data_cadastro:
                ...
                if data_cadastro:
                    st.text_input(
                        "Data de cadastro:",
                        value=data_cadastro.strftime("%d/%m/%Y %H:%M:%S"),
                        disabled=True,
                        key="data_cadastro_visualizar"
                    )
                else:
            ...
            with col_tel2:
                celular_contato_1 = st.text_input(
                    "Contato 1",
                    max_chars=15,
                    value=cliente.get("celular_contato_1", ""),
                    placeholder="(00) 90000-0000",
                    key="celular_contato_1_editar"
                )
                descricao_contato_1 = st.text_input(
                    "Quem é esse contato?",
                    max_chars=30,
                    value=cliente.get("descricao_contato_1", ""),
            ...
            with col_tel3:
                celular_contato_2 = st.text_input(
                    "Contato 2",
                    max_chars=15,
                    value=cliente.get("celular_contato_2", ""),
                    placeholder="(00) 90000-0000",
                    key="celular_contato_2_editar"
                )
                descricao_contato_2 = st.text_input(
                    "Quem é esse contato?",
                    max_chars=30,
                    value=cliente.get("descricao_contato_2", ""),
                    placeholder="Ex: Mãe",
            ...
            # --- INÍCIO MODIFICAÇÃO CONDOMÍNIO ---
            col_cpf_cond, col_rg_cond = st.columns([3, 2])
            with col_cpf_cond:
                # Checkbox e CPF na mesma linha visualmente (usando columns)
                col_cpf, col_check = st.columns([4, 1])
                with col_cpf:
                    cpf = st.text_input("CPF *", max_chars=14, placeholder="000.000.000-00", value=cliente.get("cpf", ""), key="cpf_editar")
                with col_check:
                    # Checkbox já marcado por padrão se for condomínio ou se não tiver dado
                    eh_condominio_default = cliente.get("eh_condominio", False)
                    eh_condominio = st.checkbox("É COND.", value=eh_condominio_default, key=f"eh_condominio_{key_suffix}", help="Marque se o endereço for um Condomínio")
            
            with col_rg_cond:
                rg = st.text_input("RG *", max_chars=15, placeholder="12.345.678-9", value=cliente.get("rg", ""), key="rg_editar")
            # --- FIM MODIFICAÇÃO CONDOMÍNIO ---
            ...
            # --- SE FOR CONDOMÍNIO, MOSTRAR SELEÇÃO ---
            if eh_condominio:
                st.markdown("### 🏢 Seleção de Condomínio")
                condominios_lista = carregar_condominios()
                if condominios_lista:
                    opcoes_condominios = ["Selecione..."] + [c.get("Condomínio", "") for c in condominios_lista]
                    
                    # Verifica se já tem condomínio selecionado
                    condominio_nome_atual = cliente.get("condominio_nome", "")
                    index_condominio = opcoes_condominios.index(condominio_nome_atual) if condominio_nome_atual in opcoes_condominios else 0
                    
                    condominio_selecionado = st.selectbox(
                        "Selecione o Condomínio",
                        opcoes_condominios,
                        index=index_condominio,
                        key=f"condominio_select_{key_suffix}"
                    )
                    
                    # Auto-preencher quando selecionar
                    if condominio_selecionado != "Selecione...":
                        condo_data = next((c for c in condominios_lista if c.get("Condomínio") == condominio_selecionado), None)
                        if condo_data:
                            # Armazena no session_state para usar nos campos
                            st.session_state[f"endereco_auto_{key_suffix}"] = condo_data.get("Endereço", "")
                            st.session_state[f"numero_auto_{key_suffix}"] = str(condo_data.get("Número", ""))
                            st.session_state[f"bairro_auto_{key_suffix}"] = condo_data.get("Bairro", "")
                            st.session_state[f"cidade_auto_{key_suffix}"] = condo_data.get("Cidade", "Rio de Janeiro")
                            st.session_state[f"cep_auto_{key_suffix}"] = condo_data.get("CEP", "")
                else:
                    st.warning("⚠️ Nenhum condomínio cadastrado no sistema.")
            
            col1, col2 = st.columns([3, 1])
            with col1:
                # Usa o valor do session_state se existir (auto-preenchido), senão usa o do banco
                valor_endereco = st.session_state.get(f"endereco_auto_{key_suffix}", cliente.get("endereco", ""))
                endereco = st.text_input("Endereço *", max_chars=100, value=valor_endereco, key=f"endereco_editar_{key_suffix}")
            with col2:
                valor_numero = st.session_state.get(f"numero_auto_{key_suffix}", cliente.get("numero", ""))
                numero = st.text_input("Número *", max_chars=6, value=valor_numero, key=f"numero_editar_{key_suffix}")

            col1, col2 = st.columns(2)
            with col1:
                complemento = st.text_input("Complemento", max_chars=50, value=cliente.get("complemento", ""), key="complemento_editar")
            with col2:
                valor_cidade = st.session_state.get(f"cidade_auto_{key_suffix}", cliente.get("cidade", "Paraiba do Sul"))
                cidade = st.text_input("Cidade *", max_chars=50, value=valor_cidade, key=f"cidade_editar_{key_suffix}")

            col1, col2 = st.columns(2)
            with col1:
                valor_bairro = st.session_state.get(f"bairro_auto_{key_suffix}", cliente.get("bairro", ""))
                bairro = st.text_input("Bairro *", max_chars=50, value=valor_bairro, key=f"bairro_editar_{key_suffix}")
            with col2:
                ponto_referencia = st.text_input("Ponto de referência", max_chars=100, value=cliente.get("ponto_referencia", ""), key="ponto_referencia_editar")
            ...
            with col1:
                ...
                tipo_moradia = st.selectbox(
                    "Tipo de Moradia *",
                    ["Selecione...", "Própria", "Alugada", "Cedida"],
            ...
            with col2:
                ...
                tempo_moradia_valor = st.selectbox(
                    "Tempo de Moradia",
                    list(range(0, 51)),
                    index=min(tempo_valor_atual, 50),
                    key=f"tempo_moradia_valor_{key_suffix}"
                )
                tempo_moradia_unidade = st.selectbox(
                    "",
                    ["Anos", "Meses"],
            ...
            with col2:
                if st.session_state.get("gerando_comodato_visualizar"):
                    ...
                    if st.form_submit_button("📄 Gerar Termo de Comodato", type="secondary"):
                        ...
                        if not cpf_valido:
                            st.error("❌ CPF inválido!")
                        elif not all([nome_completo, cpf_valido, endereco, celular, equip_modelo, equip_codigo]):
                            st.error("❌ Preencha todos os campos obrigatórios do comodato!")
                        else:
                            st.session_state["dados_temp_comodato_visualizar"] = {
                                "nome_contratante": nome_completo,
                                "cpf_cnpj_contratante": cpf_valido,
                                "endereco_contratante": endereco,
                                "numero_contratante": numero,
            ...
            if st.form_submit_button("📝 Gerar Mensagem de Confirmação", type="secondary"):
                ...
                campos = {
                    ...
                    "Contato 1": f"{celular_contato_1} ({descricao_contato_1})" if celular_contato_1 and descricao_contato_1 else celular_contato_1 or "—",
                    "Contato 2": f"{celular_contato_2} ({descricao_contato_2})" if celular_contato_2 and descricao_contato_2 else celular_contato_2 or "—",
                    "Email": email,
                    "Data de nascimento": data_nascimento.strftime("%d/%m/%Y") if data_nascimento else "Não informado",
                    "CPF": cpf_limpo,
                    "RG": rg,
                    "Endereço": endereco,
                    "Número": numero,
            ...
            if atualizar:
                if not all([nome_completo, celular, plano_escolhido != "Selecione..."]):
                    ...
                    if seguiu_ativacao == "Não" and (not motivo_recusa or motivo_recusa == "Selecione..."):
                        ...
                        # CORREÇÃO PRINCIPAL: Preservar campos de agendamento existentes
                        update_data = {
                            ...
                            "origem": origem if origem != "Selecione..." else "",
                            "restritivo": restritivo_valor_salvar,
                            ...
                            # PRESERVAR CAMPOS DE AGENDAMENTO (NÃO SOBRESCREVER)
                            "retorno_agendado": cliente.get("retorno_agendado", retorno_agendado),
                            "periodo": cliente.get("periodo", None),
                            "observacoes_agendamento": cliente.get("observacoes_agendamento", None),
                            "contrato_titular": cliente.get("contrato_titular", False),
                            "status_agendamento": cliente.get("status_agendamento", "agendado"),
                            "ativo": cliente.get("ativo", False),
                            "data_ativacao": cliente.get("data_ativacao", None),
                            "reagendado_para": cliente.get("reagendado_para", None),
                            "motivo_cancelamento": cliente.get("motivo_cancelamento", None),
                            "data_cancelamento": cliente.get("data_cancelamento", None),
                            
                            # CAMPOS DE CONDOMÍNIO
                            "eh_condominio": eh_condominio,
                            "condominio_nome": condominio_selecionado if eh_condominio and condominio_selecionado != "Selecione..." else None,
                            
                            "observacoes": observacoes if observacoes else "",
                            "observacoes_followup": obs_followup.strip(),
                        ...
                        if equip_modelo and equip_modelo in MODELOS_ROTEADORES:
                            update_data["equipamento_modelo"] = equip_modelo
                        if equip_codigo:
...
# ============================================================================
# EXPANDER PARA COMPLETAR CADASTRO - ATUALIZADO COM CONDOMÍNIOS
# ============================================================================
def expander_completar_cadastro(cliente, clientes_collection):
    if not cliente or not isinstance(cliente, dict):
        st.error("❌ Cliente não encontrado.")
        return
    with st.expander("✏️ Completar Cadastro do Cliente", expanded=True):
        ...
        if endereco_atual and numero_atual:
            ...
            if cliente_bloqueado:
                ...
                if cliente_bloqueado.get("observacoes_bloqueio_endereco"):
                    st.info(f"**Motivo do bloqueio:** {cliente_bloqueado['observacoes_bloqueio_endereco']}")
        ...
        with st.form("form_completar_cadastro"):
            ...
            if data_cadastro:
                ...
                if data_cadastro:
                    st.text_input(
                        ...
                        value=data_cadastro.strftime("%d/%m/%Y %H:%M:%S"),
                        disabled=True,
                        key="data_cadastro_completar"
                    )
                else:
                    st.text_input("Data de cadastro:", value="Não disponível", disabled=True, key="data_cadastro_completar")
            else:
                st.text_input("Data de cadastro:", value="Não disponível", disabled=True, key="data_cadastro_completar")
            ...
            col_tel1, col_tel2, col_tel3 = st.columns(3)
            with col_tel1:
                celular = st.text_input("Celular Principal *", max_chars=15, value=cliente["celular"], key="celular_completar")
            with col_tel2:
                celular_contato_1 = st.text_input(
                    "Contato 1",
                    max_chars=15,
                    value=cliente.get("celular_contato_1", ""),
                    placeholder="(00) 90000-0000",
                    key="celular_contato_1_completar"
                )
                descricao_contato_1 = st.text_input(
                    "Quem é esse contato?",
                    max_chars=30,
                    value=cliente.get("descricao_contato_1", ""),
                    placeholder="Ex: Esposa",
                    key="descricao_contato_1_completar"
                )
            with col_tel3:
                celular_contato_2 = st.text_input(
                    "Contato 2",
                    ...
                    value=cliente.get("celular_contato_2", ""),
                    placeholder="(00) 90000-0000",
                    key="celular_contato_2_completar"
                )
                descricao_contato_2 = st.text_input(
                    "Quem é esse contato?",
                    max_chars=30,
                    value=cliente.get("descricao_contato_2", ""),
                    placeholder="Ex: Mãe",
                    key="descricao_contato_2_completar"
                )

            nome_completo = st.text_input("Nome completo *", max_chars=80, value=cliente["nome_completo"], key="nome_completo_completar")
            ...
            with col_cpf_cond:
                col_cpf, col_check = st.columns([4, 1])
                with col_cpf:
                    cpf = st.text_input("CPF *", max_chars=14, placeholder="000.000.000-00", value=cliente.get("cpf", ""), key="cpf_completar")
                with col_check:
                    eh_condominio_default = cliente.get("eh_condominio", False)
                    eh_condominio = st.checkbox("É COND.", value=eh_condominio_default, key=f"eh_condominio_{key_suffix}")
            
            with col_rg_cond:
                rg = st.text_input("RG *", max_chars=15, placeholder="12.345.678-9", value=cliente.get("rg", ""), key="rg_completar")
            # --- FIM MODIFICAÇÃO CONDOMÍNIO ---
            ...
            if data_nascimento_str:
                try:
                    ...
                    pass
            data_nascimento = st.date_input("Data de nascimento *", value=data_nascimento, format="DD/MM/YYYY", key="data_nascimento_completar", min_value=datetime(1900, 1, 1))

            email = st.text_input("Email *", max_chars=50, value=cliente.get("email", ""), key="email_completar")
            ...
            with col1:
                valor_endereco = st.session_state.get(f"endereco_auto_{key_suffix}", cliente.get("endereco", ""))
                endereco = st.text_input("Endereço *", max_chars=100, value=valor_endereco, key=f"endereco_completar_{key_suffix}")
            with col2:
                valor_numero = st.session_state.get(f"numero_auto_{key_suffix}", cliente.get("numero", ""))
                numero = st.text_input("Número *", max_chars=6, value=valor_numero, key=f"numero_completar_{key_suffix}")

            col1, col2 = st.columns(2)
            with col1:
                complemento = st.text_input("Complemento", max_chars=50, value=cliente.get("complemento", ""), key="complemento_completar")
            with col2:
                valor_cidade = st.session_state.get(f"cidade_auto_{key_suffix}", cliente.get("cidade", "Paraiba do Sul"))
                cidade = st.text_input("Cidade *", max_chars=50, value=valor_cidade, key=f"cidade_completar_{key_suffix}")

            col1, col2 = st.columns(2)
            with col1:
                valor_bairro = st.session_state.get(f"bairro_auto_{key_suffix}", cliente.get("bairro", ""))
                bairro = st.text_input("Bairro *", max_chars=50, value=valor_bairro, key=f"bairro_completar_{key_suffix}")
            with col2:
                ponto_referencia = st.text_input("Ponto de referência", max_chars=100, value=cliente.get("ponto_referencia", ""), key="ponto_referencia_completar")
            ...
            plano_atual = cliente.get("plano_escolhido")
            index_plano = (PLANOS.index(plano_atual) + 1) if plano_atual in PLANOS else 0
            plano_escolhido = st.selectbox("Plano escolhido *", ["Selecione..."] + PLANOS, index=index_plano, key="plano_escolhido_completar")

            profissao = st.text_input("Profissão *", max_chars=50, value=cliente.get("profissao", ""), key="profissao_completar")

            data_vencimento = st.selectbox("Melhor data de vencimento *", list(range(1, 32)), key="data_vencimento_completar")

            codigo_indicador_atual = cliente.get("codigo_indicador", "")
            codigo_indicador = st.text_input("Código de Quem Indicou", max_chars=15, value=codigo_indicador_atual, key="codigo_indicador_completar")
                ...
                if st.session_state.get("gerando_contrato_completar"):
                    ...
                    if st.form_submit_button("✍️ Gerar Contrato", type="secondary"):
                        ...
                        if not cpf_valido:
                            ...
                            st.session_state["dados_temp_contrato_completar"] = {
                                "nome_contratante": nome_completo,
                                "cpf_cnpj_contratante": cpf_valido,
                                "endereco_contratante": endereco,
                                "numero_contratante": numero,
                            ...
                            st.session_state["gerando_contrato_completar"] = True
                            st.session_state["nome_arquivo_contrato_completar"] = f"Contrato_{nome_completo.replace(' ', '_')}.pdf"
                            st.rerun()
                ...
                if st.session_state.get("gerando_comodato_completar"):
                    ...
                    if st.form_submit_button("📄 Gerar Termo de Comodato", type="secondary"):
                        ...
                        if not cpf_valido:
                            ...
                            st.session_state["dados_temp_comodato_completar"] = {
                                "nome_contratante": nome_completo,
                                "cpf_cnpj_contratante": cpf_valido,
                                "endereco_contratante": endereco,
                                "numero_contratante": numero,
                            ...
                            st.session_state["gerando_comodato_completar"] = True
                            st.session_state["nome_arquivo_comodato_completar"] = f"Termo_Comodato_{nome_completo.replace(' ', '_')}.pdf"
                            st.rerun()
            ...
            if st.form_submit_button("📝 Gerar Mensagem de Confirmação", type="secondary"):
                ...
                for chave, valor in campos.items():
                    mensagem += f"{chave}: {valor}\n"
                mensagem += "\nAguardamos sua resposta para prosseguirmos. Qualquer dúvida, estou à disposição!"
                st.session_state["mensagem_confirmacao_completar"] = mensagem
                st.success("✅ Mensagem gerada!")

            atualizar = st.form_submit_button("🔄 Atualizar para Cadastro Completo", type="primary")
            if atualizar:
                if not all([nome_completo, celular, plano_escolhido != "Selecione..."]):
                    st.error("⚠️ Nome, Celular e Plano são obrigatórios!")
                else:
                    if seguiu_ativacao == "Não" and (not motivo_recusa or motivo_recusa == "Selecione..."):
                        ...
                        if foto_documento:
                            foto_bytes = foto_documento.read()
                            foto_base64 = base64.b64encode(foto_bytes).decode('utf-8')
                        celular_normalizado = normalize_phone(celular)
                        restritivo_valor_salvar = restritivo if restritivo != "Selecione..." else ""
                        codigo_indicacao = cliente.get("codigo_indicacao")
                        if seguiu_ativacao == "Sim" and not codigo_indicacao:
                            codigo_indicacao = gerar_codigo_indicacao()
                        ...
                        try:
                            clientes_collection.update_one({"_id": cliente["_id"]}, {"$set": update_data})
                            st.success("✅ Cadastro atualizado para COMPLETO!")
                            st.balloons()
                            st.session_state["mostrar_completar"] = False
                            if "mensagem_confirmacao_completar" in st.session_state:
                                del st.session_state["mensagem_confirmacao_completar"]
...
def render_cadastro(clientes_collection):
    ...
    if "ultimo_cpf" not in st.session_state:
        st.session_state["ultimo_cpf"] = ""
    if "ultimo_celular" not in st.session_state:
        st.session_state["ultimo_celular"] = ""
                        ...
                        if data_cadastro:
                            if isinstance(data_cadastro, str):
                                try:
                                    ...
                                    data_cadastro = None
                            if data_cadastro:
                                st.write(f"**Data de Cadastro:** {data_cadastro.strftime('%d/%m/%Y %H:%M:%S')}")
                            else:
    ...
    acao = st.radio(
        ...
        index=0 if st.session_state["acao_selecionada"] == "Novo Cadastro" else 1,
        horizontal=True,
    ...
    if acao == "Novo Cadastro" and not st.session_state.get("mostrar_visualizar", False) and not st.session_state.get("mostrar_completar", False):
        ...
        with col_tel1:
            celular_principal = st.text_input(
                ...
                on_change=verificar_duplicidade
            )
        with col_tel2:
            celular_contato_1 = st.text_input(
                "Contato 1",
                ...
                key=f"campo_celular_contato_1_{st.session_state['form_key']}"
            )
            descricao_contato_1 = st.text_input(
                "Quem é esse contato?",
                max_chars=30,
        ...
        with col_tel3:
            celular_contato_2 = st.text_input(
                ...
                key=f"campo_celular_contato_2_{st.session_state['form_key']}"
            )
            descricao_contato_2 = st.text_input(
                "Quem é esse contato?",
                max_chars=30,
        ...
        if cliente_existente:
            msg_erro = "❌ CPF já cadastrado no sistema!" if campo_duplicado == "cpf" else "❌ Celular já cadastrado no sistema!"
            st.error(msg_erro)
            st.info(f"💡 Cliente: **{cliente_existente.get('nome_completo', 'Nome não disponível')}**")
            st.warning("Use a busca global para encontrar e atualizar o cadastro existente.")
            if st.button("🔗 Abrir cadastro existente", key=f"abrir_existe_global_{st.session_state['form_key']}"):
                st.session_state["acao_selecionada"] = "Completar Cadastro Existente"
                st.session_state["busca_pre_preenchida"] = cliente_existente["celular"]
                st.rerun()

        # --- INÍCIO MODIFICAÇÃO CONDOMÍNIO (NOVO CADASTRO) ---
        if tipo_cadastro == "Cadastro CRM":
            ...
            with col_cpf_cond:
                ...
                with col_check:
                    eh_condominio = st.checkbox("É COND.", value=False, key=f"eh_condominio_novo_{st.session_state['form_key']}")
        ...
        if tipo_cadastro == "Cadastro CRM":
            with st.container(border=True):
                ...
                with col_auto:
                    followup_opcao = st.selectbox(
                        ...
                        ["Nenhum", "1 dia", "3 dias", "5 dias", "10 dias"],
                ...
                with col_manual:
                    ...
                    data_personalizada = st.date_input(
                        "Ou escolha uma data específica:",
                        value=data_personalizada,
                ...
                observacoes_followup_simples = st.text_area(
                    ...
                    key=f"observacoes_followup_novo_{st.session_state['form_key']}"
                )
            ...
            with st.container(border=True):
                ...
                with col_manual:
                    ...
                    data_personalizada_simples = st.date_input(
                        "Ou escolha uma data específica:",
                        value=data_personalizada_simples,
                        min_value=datetime.today().date(),
                        format="DD/MM/YYYY",
        ...
        # --- SE FOR CONDOMÍNIO, MOSTRAR SELEÇÃO (NOVO CADASTRO) ---
        if tipo_cadastro == "Cadastro CRM" and eh_condominio:
            st.markdown("### 🏢 Seleção de Condomínio")
            condominios_lista = carregar_condominios()
            if condominios_lista:
                opcoes_condominios = ["Selecione..."] + [c.get("Condomínio", "") for c in condominios_lista]
                ...
                if condominio_selecionado != "Selecione...":
                    condo_data = next((c for c in condominios_lista if c.get("Condomínio") == condominio_selecionado), None)
                    if condo_data:
                        st.session_state[f"endereco_auto_novo_{st.session_state['form_key']}"] = condo_data.get("Endereço", "")
                        st.session_state[f"numero_auto_novo_{st.session_state['form_key']}"] = str(condo_data.get("Número", ""))
                        st.session_state[f"bairro_auto_novo_{st.session_state['form_key']}"] = condo_data.get("Bairro", "")
                        st.session_state[f"cidade_auto_novo_{st.session_state['form_key']}"] = condo_data.get("Cidade", "Rio de Janeiro")
                        st.session_state[f"cep_auto_novo_{st.session_state['form_key']}"] = condo_data.get("CEP", "")
            else:
                st.warning("⚠️ Nenhum condomínio cadastrado no sistema.")
                    ...
                    if data_cad:
                        try:
                            if isinstance(data_cad, str):
                                data_cad = datetime.fromisoformat(data_cad)
        ...
        with st.form(f"novo_cadastro_{st.session_state['form_key']}"):
            if tipo_cadastro == "Cadastro CRM":
                # RG e Data Nascimento já foram definidos acima nas columns
                ...
                col1, col2 = st.columns([3, 1])
                with col1:
                    # Usa o valor do session_state se existir (auto-preenchido), senão usa o inicial
                    valor_endereco = st.session_state.get(f"endereco_auto_novo_{st.session_state['form_key']}", get_valor_inicial("endereco", ""))
                    endereco = st.text_input(
                        "Endereço *",
                        value=valor_endereco,
                        key=f"endereco_{st.session_state['form_key']}"
                    )
                with col2:
                    valor_numero = st.session_state.get(f"numero_auto_novo_{st.session_state['form_key']}", get_valor_inicial("numero", ""))
                ...
                with col2:
                    valor_cidade = st.session_state.get(f"cidade_auto_novo_{st.session_state['form_key']}", get_valor_inicial("cidade", "Paraiba do Sul"))
                ...
                col1, col2 = st.columns(2)
                with col1:
                    valor_bairro = st.session_state.get(f"bairro_auto_novo_{st.session_state['form_key']}", get_valor_inicial("bairro", ""))
                ...
                with col2:
                    tempo_moradia_valor = st.selectbox(
                        ...
                        list(range(0, 51)),
                        index=0,
                        key=f"tempo_moradia_valor_{st.session_state['form_key']}"
                    )
                    tempo_moradia_unidade = st.selectbox(
                        "",
                        ["Anos", "Meses"],
                ...
                if st.session_state["gerando_comodato_principal"]:
                    ...
                    if st.form_submit_button("📄 Gerar Termo de Comodato", type="secondary"):
                        ...
                        if not cpf_valido:
                            ...
                            st.session_state["dados_temp_comodato_principal"] = {
                                "nome_contratante": nome_completo,
                                "cpf_cnpj_contratante": cpf_valido,
                                "endereco_contratante": endereco,
                                "numero_contratante": numero,
            ...
            if enviado:
                if not nome_completo or not celular_principal:
                    ...
                    # Recupera o nome do condomínio selecionado se houver
                    condominio_nome_salvo = None
                    if tipo_cadastro == "Cadastro CRM" and eh_condominio:
                        condominio_selecionado_novo = st.session_state.get(f"condominio_select_novo_{st.session_state['form_key']}", "Selecione...")
                        if condominio_selecionado_novo != "Selecione...":
                            condominio_nome_salvo = condominio_selecionado_novo
                    ...
                    cliente_data = {
                        ...
                        # CAMPOS DE CONDOMÍNIO
                        "eh_condominio": eh_condominio if tipo_cadastro == "Cadastro CRM" else False,
                        "condominio_nome": condominio_nome_salvo,
                    }
    ...
    if st.session_state.get("mostrar_botao_novo", False):
        if st.button("🔄 Novo Cadastro", key="btn_novo_cadastro_global"):
            keys_to_clear = [
                "campo_cpf", "campo_celular_principal", "campo_celular_contato_1", "campo_celular_contato_2",
                "descricao_contato_1", "descricao_contato_2",
                "nome_completo", "email", "data_nascimento", "rg", "endereco", "numero", "bairro",
                "ponto_referencia", "plano_escolhido", "profissao",
                "data_vencimento", "foto_documento",

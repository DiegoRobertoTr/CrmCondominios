    ...
    if acao == "Novo Cadastro" and not st.session_state.get("mostrar_visualizar", False) and not st.session_state.get("mostrar_completar", False):
        ...
        with st.form(f"novo_cadastro_{st.session_state['form_key']}"):
            if tipo_cadastro == "Cadastro CRM":
                ...
                # ✅ CORREÇÃO CRÍTICA: Removido 'value' dos campos que usam session_state
                col1, col2 = st.columns([3, 1])
                with col1:
                    endereco = st.text_input(
                        "Endereço*",
                        key=f"endereco_{st.session_state['form_key']}"  # Removido 'value'
                    )
                with col2:
                    numero = st.text_input(
                        "Número*",
                        max_chars=6,
                        key=f"numero_{st.session_state['form_key']}"  # Removido 'value'
                    )
                ...
                with col_bloco:
                    bloco = st.text_input(
                        "Bloco",
                        key=f"bloco_{st.session_state['form_key']}"  # Removido 'value'
                    )
                with col_apto:
                    apartamento = st.text_input(
                        "Apartamento",
                        key=f"apartamento_{st.session_state['form_key']}"  # Removido 'value'
                    )
                ...
                with col1:
                    complemento = st.text_input(
                        "Complemento",
                        key=f"complemento_{st.session_state['form_key']}"  # Removido 'value'
                    )
                with col2:
                    ponto_referencia = st.text_input(
                        "Ponto de referência",
                        key=f"ponto_referencia_{st.session_state['form_key']}"  # Removido 'value'
                    )
                ...
                with col1:
                    bairro = st.text_input(
                        "Bairro*",
                        key=f"bairro_{st.session_state['form_key']}"  # Removido 'value'
                    )
                with col2:
                    cidade = st.text_input(
                        "Cidade*",
                        key=f"cidade_{st.session_state['form_key']}"  # Removido 'value'
                    )
                ...
                with col1:
                    cep = st.text_input(
                        ...
                        key=f"cep_{st.session_state['form_key']}"  # Removido 'value'
                    )
                with col2:
                    pass
                ...
                bloco = st.session_state.get(f"bloco_{st.session_state['form_key']}", "") or ""
                apartamento = st.session_state.get(f"apartamento_{st.session_state['form_key']}", "") or ""
                complemento = st.session_state.get(f"complemento_{st.session_state['form_key']}", "") or ""
                cidade = st.session_state.get(f"cidade_{st.session_state['form_key']}", "") or ""

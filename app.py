...
# ✅ Lógica para exibir APENAS a linha divisória (sem labels)
...
for i, modulo in enumerate(opcoes_modulos):
    ...
    if modulo in grupo_condominios:
        # Se o item anterior NÃO for deste grupo, desenha APENAS a linha
        if i > 0 and opcoes_modulos[i-1] not in grupo_condominios:
            st.sidebar.divider()  # ← Apenas a linha, sem texto!
